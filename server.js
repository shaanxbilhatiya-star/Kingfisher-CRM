...
//               PEER_CRM_2=https://swiggy-crm.up.railway.app
...
function generateDailyReport(dateStr) {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.rect(0, 0, pageWidth, 120).fill(BRAND_BLUE);
      doc.rect(0, 120, pageWidth, 4).fill('#FACC15'); // Amber accent line

      doc.fillColor('#FFFFFF')
         .fontSize(24)
         .font('Helvetica-Bold')
         .text('KINGFISHER MANDLA', 40, 40);
      
      doc.fillColor('#BFDBFE')
         .fontSize(13)
         .font('Helvetica')
         .text('x  Ruralift', 40, 70);
      ...
      const converted = stats.interestedCount || 0;

      // Calculate revenue from interested leads (mock calculation for demo)
      const estimatedRevenue = converted * 25000; // Average Rs. 25,000 per conversion
      ...
      const kpis = [
        ...
        { label: 'CONVERTED', value: converted, color: PINK },
        { label: 'REVENUE', value: `₹${Math.round(estimatedRevenue / 1000)}K`, color: '#16A34A' }
      ];
      ...
      kpis.forEach((kpi, i) => {
        ...
        const fontSize = valueStr.length > 5 ? 18 : 22;
        doc.fillColor(kpi.color)
           .fontSize(fontSize)
           .font('Helvetica-Bold')
           .text(valueStr, x, y + 25, { width: cardWidth, align: 'center' });
      ...
      const convRate = totalDialed ? Math.round((converted / totalDialed) * 100) : 0;
      ...
      doc.fillColor(BRAND_BLUE_DARK)
         .fontSize(11)
         .font('Helvetica-Bold')
         .text(`Conversion Rate: ${convRate}%`, 60, y + 12)
      ...
      doc.rect(40, y + 20, 10, 3).fill(BRAND_BLUE);
      ...
      const statsList = [
        ...
        ['Interested Leads', stats.interestedCount || 0],
        ['Overdue (72h+)', stats.overdueInterestedCount || 0],
      ...
      const addFooter = () => {
        ...
        doc.fillColor(MUTED)
           .fontSize(8)
           .font('Helvetica')
           .text('Kingfisher Mandla x Ruralift  |  Daily Agent Report', 40, bottomY + 10);
      ...
      if (activeDispos.length === 0) {
        ...
        activeDispos.forEach(key => {
          ...
          const rows = phones.slice(0, 20).map((p, i) => [String(i + 1), p]);
          y = drawTable(y, ['#', 'Phone Number'], rows, color, '#F8FAFC');
          y += 5;
        });
      }

      // Section: Interested Leads (Today)
      const interestedLeads = appState.numbers.filter(n => 
      ...
      if (interestedLeads.length > 0) {
        ...
        doc.fillColor(GREEN)
           .fontSize(16)
           .font('Helvetica-Bold')
           .text('Interested Leads (Today)', 40, y);
        doc.rect(40, y + 20, 10, 3).fill(GREEN);
        y += 30;

        const intRows = interestedLeads.slice(0, 15).map(n => [
          (n.phone || 'N/A').slice(0, 10),
          (n.name || 'N/A').slice(0, 15),
          (n.loanType || 'N/A').slice(0, 15),
          `₹${((n.loanAmount || 0) / 1000).toFixed(0)}K`
        ]);

        y = drawTable(y, ['Phone', 'Name', 'Package', 'Revenue'], intRows, GREEN, '#ECFDF5');
      }

      // Section: Follow-ups (Scheduled)
      const followupLeads = appState.numbers.filter(n => n.disposition === 'followup' && n.followupDate);
      ...
      if (followupLeads.length > 0) {
        ...
        const fuRows = followupLeads.slice(0, 15).map(n => [
          (n.phone || 'N/A').slice(0, 10),
          (n.name || 'N/A').slice(0, 15),
          (n.loanType || 'N/A').slice(0, 15),
        ...
        y = drawTable(y, ['Phone', 'Name', 'Package', 'Date', 'Time'], fuRows, BLUE, '#EFF6FF');
      }
      ...
      // Section: Converted Customers (Today)
      const convertedToday = appState.numbers.filter(n => 
        n.disposition === 'interested' && n.dialedAt && n.dialedAt.slice(0, 10) === dateStr
      );
      ...
      if (convertedToday.length > 0) {
        ...
        doc.fillColor(PINK)
           .fontSize(16)
           .font('Helvetica-Bold')
           .text('Converted Customers (Today)', 40, y);
        doc.rect(40, y + 20, 10, 3).fill(PINK);
        y += 30;

        const totalRevenue = convertedToday.reduce((sum, n) => sum + (n.loanAmount || 0), 0);
        doc.roundedRect(40, y, contentWidth, 25, 6).fill('#FDF2F8');
        doc.fillColor('#BE185D')
           .fontSize(11)
           .font('Helvetica-Bold')
           .text(`Total Revenue Generated: ₹${totalRevenue.toLocaleString('en-IN')}`, 50, y + 10);
        y += 32;

        const convRows = convertedToday.slice(0, 15).map(n => [
          (n.phone || 'N/A').slice(0, 10),
          (n.name || 'N/A').slice(0, 15),
          (n.loanType || 'N/A').slice(0, 15),
          `₹${((n.loanAmount || 0) / 1000).toFixed(0)}K`,
          n.dialedAt ? n.dialedAt.slice(0, 10) : 'N/A'
        ]);

        y = drawTable(y, ['Phone', 'Name', 'Package', 'Revenue', 'Date'], convRows, PINK, '#FDF2F8');
      }
