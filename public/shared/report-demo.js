/* ============================================================================
   Kingfisher Mandla x Ruralift CRM — Shared "Sample / Demo" Report Generator
   ----------------------------------------------------------------------------
   Produces a beautifully designed, Kingfisher-styled PDF report populated with
   realistic dummy data. Used everywhere a "Download Sample Report" button is
   shown (Generated Reports Archive on the admin panel, etc.) so users always
   have something gorgeous to download even before the first real report is
   auto-generated at 6:30 PM IST.

   Fully namespaced under  window.KingfisherReport  to avoid any global clashes.
   Mirrors the live agent Daily Report look so the sample is a faithful preview.
...
(function () {
  ...
  // ── Kingfisher brand palette (RGB) — matches the live agent report ──────────
  var BRAND_BLUE      = [37, 99, 235];    // #2563EB
  var BRAND_BLUE_DARK = [29, 78, 216];    // #1D4ED8
  var INK             = [30, 41, 59];     // #1E293B
  ...
  var DISPO_META = {
    ...
    'Discard':        { color: [107, 114, 128], label: 'Discard' }
  };
  ...
  function addFooters(doc) {
    ...
    for (var i = 1; i <= total; i++) {
      ...
      doc.setTextColor(MUTED[0], MUTED[1], MUTED[2]);
      doc.text('Kingfisher Mandla x Ruralift  |  Sample / Demo Report', 14, 291);
  ...
  function demoData() {
    return {
      ...
      interested: [
        ['98•••••210', 'Rahul Verma',   'Personal Loan',  'Rs. 25,000'],
        ['97•••••884', 'Amit Sharma',   'Business Loan',  'Rs. 40,000'],
        ['90•••••173', 'Sohel Khan',    'Gold Loan',      'Rs. 18,000'],
        ['88•••••452', 'Pradeep Nair',  'Agri Loan',      'Rs. 32,000'],
        ['96•••••019', 'Vikas Yadav',   'Personal Loan',  'Rs. 21,000'],
        ['70•••••638', 'Manish Gupta',  'Business Loan',  'Rs. 55,000']
      ],
      followups: [
        ['98•••••771', 'Deepak Rao',    'Gold Loan',      '05-07-2026', '11:30 AM'],
        ['91•••••205', 'Suresh Menon',  'Personal Loan',  '05-07-2026', '02:15 PM'],
        ['99•••••640', 'Arjun Singh',   'Agri Loan',      '06-07-2026', '10:00 AM'],
        ['80•••••317', 'Farhan Ali',    'Business Loan',  '06-07-2026', '04:45 PM']
      ],
      converted: [
        ['98•••••210', 'Rahul Verma',   'Personal Loan',  'Rs. 25,000', '04-07-2026'],
        ['90•••••173', 'Sohel Khan',    'Gold Loan',      'Rs. 18,000', '04-07-2026'],
        ['96•••••019', 'Vikas Yadav',   'Personal Loan',  'Rs. 21,000', '04-07-2026']
      ]
    };
  }
  ...
  function addInterestedSection(doc, y, leads) {
    y = sectionHeader(doc, y, 'Interested Leads (Today)', [16, 185, 129]);
    doc.autoTable({
      startY: y,
      head: [['Phone', 'Name', 'Package', 'Revenue (Rs.)']],
      body: leads,
      theme: 'striped',
  ...
  function addFollowupsSection(doc, y, leads) {
    y = sectionHeader(doc, y, 'Follow-ups (Today)', [59, 130, 246]);
    doc.autoTable({
      startY: y,
      head: [['Phone', 'Name', 'Package', 'Date', 'Time']],
      body: leads,
      theme: 'striped',
  ...
  function addConvertedSection(doc, y, leads) {
    y = sectionHeader(doc, y, 'Converted Customers (Today)', [236, 72, 153]);
    var totalRev = leads.reduce(function (s, l) { return s + parseRupees(l[3]); }, 0);
    doc.setFillColor(253, 242, 248);
    doc.roundedRect(14, y - 3, 182, 9, 2, 2, 'F');
    doc.setFontSize(10); doc.setFont('helvetica', 'bold');
    doc.setTextColor(190, 24, 93);
    doc.text('Total Revenue Generated:  Rs. ' + totalRev.toLocaleString('en-IN'), 18, y + 3);
    doc.setTextColor(INK[0], INK[1], INK[2]);
    doc.setFont('helvetica', 'normal');
    y += 11;
    doc.autoTable({
      startY: y,
      head: [['Phone', 'Name', 'Package', 'Revenue (Rs.)', 'Date']],
      body: leads,
      theme: 'striped',
  ...
  function build() {
    ...
    var fileName = dateStr + '_Kingfisher_Report_SAMPLE.pdf';
    ...
    var revenue     = d.converted.reduce(function (s, row) { return s + parseRupees(row[3]); }, 0);
    ...
    // ===== HEADER (Kingfisher gradient) =====
    gradientRect(doc, 0, 0, 210, 42, BRAND_BLUE_DARK, BRAND_BLUE);
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(21);
    doc.text('KINGFISHER MANDLA', 14, 17);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(11);
    doc.setTextColor(191, 219, 254);
    doc.text('x  Ruralift', 14, 25);
    ...
    doc.roundedRect(160, 31, 36, 6.5, 1.5, 1.5, 'F');
    doc.setTextColor(BRAND_BLUE_DARK[0], BRAND_BLUE_DARK[1], BRAND_BLUE_DARK[2]);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(7.5);
    doc.text('SAMPLE / DEMO DATA', 178, 35.3, { align: 'center' });
    ...
    var y = 52;
    var kpis = [
      { label: 'Total Dialed', value: totalDialed, color: BRAND_BLUE },
      { label: 'Interested',   value: interestedN, color: [16, 185, 129] },
      { label: 'Follow-ups',   value: followupN,   color: [59, 130, 246] },
      { label: 'Converted',    value: convertedN,  color: [236, 72, 153] },
      { label: 'Revenue',      value: 'Rs ' + revenue.toLocaleString('en-IN'), color: [22, 163, 74] }
    ];
    ...
    doc.setTextColor(BRAND_BLUE_DARK[0], BRAND_BLUE_DARK[1], BRAND_BLUE_DARK[2]);
    doc.text('Conversion Rate: ' + convRate + '%', 18, y + 5.9);
    ...
    y = sectionHeader(doc, y, 'Call Disposition Breakdown', BRAND_BLUE);
    ...
    // ===== FUNNEL BAR CHART =====
    y = sectionHeader(doc, y, 'Performance Funnel', BRAND_BLUE);
    var funnelImg = makeChartImage({
      type: 'bar',
      data: {
        labels: ['Dialed', 'Interested', 'Follow-ups', 'Converted'],
        datasets: [{
          data: [totalDialed, interestedN, followupN, convertedN],
          backgroundColor: [rgb(BRAND_BLUE), rgb([16, 185, 129]), rgb([59, 130, 246]), rgb([236, 72, 153])],
    ...
    y = addInterestedSection(doc, y, d.interested);
    y = addFollowupsSection(doc, y, d.followups);
    y = addDndSection(doc, y, dndNumbers);
    y = addConvertedSection(doc, y, d.converted);
